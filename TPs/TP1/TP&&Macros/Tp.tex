...
\begin{proc}{grandesCiudades}{\In ciudades : \TLista{\mathsf{Ciudad}}}{\TLista{\mathsf{Ciudad}}}
    ...
    \asegura{\mathsf{noHayNombresRepetidos}(res) \land ((c \in ciudades \land c.habitantes > 50000) \leftrightarrow c \in res)}
\end{proc}
...
%     \> \texttt{requiere} \{$\mathsf{esMatrizCuadrada}(distancias) \land \mathsf{esMatrizDiagonal}(distancias)$\} \\
...
% \begin{tabbing}
%     tabula \=tabula \=tabula \kill 
%     \texttt{pred esMatrizDiagonal} $(m : seq \langle seq \langle \mathbb{Z} \rangle \rangle)$ \{ \\
...
\begin{proc}{hayCamino}{\In distancias : \TLista{\TLista{\ent}}, \In desde, hasta : \ent}{\textsf{Bool}}
    \requiere{\mathsf{esMatrizCuadrada}(distancias) \land \mathsf{esMatrizDiagonal}(distancias)}
...
% \pred{esMatrizDiagonal}{m : \TLista{\TLista{\ent}}}{\paraTodo[unalinea]{i, j}{\ent}{0 \leq i, j < |m| \longrightarrow_L m[i][j] = m[j][i]}}
...
\paraTodo[unalinea]{i}{\ent}{0 \leq i < |camino| - 1 \longrightarrow_L (camino[i] \neq camino[i + 1] \longrightarrow m[i][i + 1] > 0)}}
...
\begin{proc}{cantidadCaminosNSaltos}{\Inout conexion : \TLista{\TLista{\ent}}, \In n : \ent}{}
	\requiere{conexion = C_0 \land \mathsf{esMatrizCuadrada}(conexion) \land \mathsf{esMatrizDiagonal}(conexion) \land n \geq 1}
    \requiere{\paraTodo[unalinea]{i,j}{\ent}{0 \leq i, j < |conexion| \longrightarrow_L 0 \leq conexion[i][j] \leq 1}}
	\asegura{\mathsf{esMatrizCuadrada}(conexion) \land \mathsf{esMatrizDiagonal}(conexion) \land \mathsf{esNEsimaPotencia}(conexion, \ C_0, \ n)}
\end{proc}

\pred{esNEsimaPotencia}{matriz, base : \TLista{\TLista{\ent}}, exp : \ent}
{\existe[unalinea]{s}{\TLista{\TLista{\TLista{\ent}}}}{|s| = exp \land s[0] = base \land s[exp - 1] = matriz \land_L \\
\paraTodo[unalinea]{i}{\ent}{1 \leq i < exp \longrightarrow_L \mathsf{esProducto}(s[i], \ s[i-1], \ s[0])}}}
...
%     \> \texttt{requiere} \{$0 \leq origen, destino < |distancias| \land \mathsf{esMatrizCuadrada}(distancias) \land \mathsf{esMatrizDiagonal}(distancias)$\} \\
...
\textcolor{darkgray}{(\textsc{NOTA}: Reusamos aquí los predicados \textsf{esMatrizCuadrada}, \textsf{esMatrizDiagonal} y \textsf{ningunElementoNegativo}.)}
...
\aux{distanciaCamino}{camino : \TLista{\ent}, m : \TLista{\TLista{\ent}}}{\ent}{\sum\limits_{i = 0}^{|camino| - 1} m[i][i + 1]}
